...
include('db/conexion.php');
		...
		if(isset($_REQUEST['codigo_estudiante'])){
			...
			$datos_tipoidentificacion=pg_fetch_array($query_tipoidentificacion);
					$tid_codigo_m=$datos_tipoidentificacion['tid_codigo'];
					$tid_referencia=$datos_tipoidentificacion['tid_referencia'];
					$tid_nombre=$datos_tipoidentificacion['tid_nombre'];
?>
<html>
	<head>
			...
			<option value='0'>Escoja</option>
					...
					<?php
						while($data_tipoidentificacion=pg_fetch_array($query_tipoidentificacion)){
							...
							echo "<option value='$tid_codigo'>$tid_nombre ($tid_referencia)</option>";
						}
					?>
					</select>
				</div>
